...
class VideoTokenizerTrainer:
    ...
    def __init__(
        ...
        *,
        ...
        self.warmup = LinearWarmup(self.optimizer, warmup_period=warmup_steps)

        self.discr_warmup = LinearWarmup(self.discr_optimizer, warmup_period=warmup_steps)
        ...
        if exists(scheduler):
            ...
            self.scheduler = paddle.optimizer.lr.LambdaDecay(
                learning_rate=self.optimizer.get_lr(), lr_lambda=lambda step: 1.0
            )
            self.optimizer.set_lr_scheduler(self.scheduler)

            self.discr_scheduler = paddle.optimizer.lr.LambdaDecay(
                learning_rate=self.discr_optimizer.get_lr(), lr_lambda=lambda step: 1.0
            )
            self.discr_optimizer.set_lr_scheduler(self.discr_scheduler)
