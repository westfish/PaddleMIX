...
class VideoTokenizer(paddle.nn.Layer):
    ...
    def __init__(
        ...
        *,
        ...
        multiscale_discrs: Tuple[paddle.nn.Layer, ...] = tuple(),  # do not support multiscale discrs
        use_gan=False,  # do not support use_gan
    ...
    @paddle.no_grad()
    def log_reconstruct_frames(self, video):
        self.eval()
        codes = self.tokenize(video)
        sampled_videos = self.decode_from_code_indices(codes)
    ...
    @paddle.no_grad()
    def log_original_frames(self, video):
        self.eval()
        sampled_videos = video
