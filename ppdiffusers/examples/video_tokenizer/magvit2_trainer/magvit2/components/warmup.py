...
class BaseWarmup(object):
    ...
    def __init__(self, optimizer, warmup_params, last_step=-1):
        ...
        self.lrs = [group["learning_rate"] for group in self.optimizer._param_groups]
        self.dampen()
    ...
    def dampen(self, step=None):
        ...
        self.last_step = step
        for group, params in zip(self.optimizer._param_groups, self.warmup_params):
            omega = self.warmup_factor(step, **params)
            group["learning_rate"] *= omega

    @contextmanager
    def dampening(self):
        if self.last_step > 0:
            for group, lr in zip(self.optimizer._param_groups, self.lrs):
...
class LinearWarmup(BaseWarmup):
    ...
    def __init__(self, optimizer, warmup_period, last_step=-1):
        group_count = len(optimizer._param_groups)
