...
def tgate(
    ...
    **kwargs,
    ...
    if self.do_classifier_free_guidance:
        cfg_embeds = paddle.concat([negative_prompt_embeds, prompt_embeds], axis=0)
        cfg_add_text_embeds = paddle.concat([negative_pooled_prompt_embeds, add_text_embeds], axis=0)
        cfg_add_time_ids = paddle.concat([negative_add_time_ids, add_time_ids], axis=0)
        cfg_embeds = cfg_embeds
        cfg_add_text_embeds = cfg_add_text_embeds
        cfg_add_time_ids = cfg_add_time_ids.tile([batch_size * num_images_per_prompt, 1])
        negative_prompt_embeds = negative_prompt_embeds
        negative_add_text_embeds = negative_pooled_prompt_embeds
        negative_add_time_ids = negative_add_time_ids.tile([batch_size * num_images_per_prompt, 1])
    else:
