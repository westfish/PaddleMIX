...
class InputOutputPredictor:
    ...
    def __init__(self, strategy='adaptive_learning', max_samples=50):
        ...
        # 动态选择最佳模型
        self.model_scores = {}
        self.current_best_model = 'polynomial_2'
        
    def _init_models(self):
        """初始化不同的预测模型"""
        # 多项式回归模型（不同阶数）
        for degree in range(1, 4):
            self.models[f'polynomial_{degree}'] = Pipeline([
        ...
        for model_name in self.models.keys():
            self.model_scores[model_name] = {'mse': float('inf'), 'count': 0}
    ...
    def predict_output_change(self, input_change):
        ...
        try:
            ...
            pred = model.predict([[input_change]])[0]
    ...
    def _heuristic_prediction(self, input_change):
        ...
        # 使用历史数据的简单统计
        avg_ratio = np.mean([
            o / (i + 1e-8) for i, o in zip(self.input_changes, self.output_changes)
        ])
...
class AdaptiveThresholdManager:
    ...
    def __init__(self, initial_thresh=0.01):
        ...
        self.prediction_error_history = collections.deque(maxlen=15)
    ...
    def update(self, was_cached, prediction_error=None):
        ...
        # 基于缓存命中率调整
        if len(self.cache_hit_history) >= 10:
            current_hit_rate = np.mean(self.cache_hit_history)
            
            if current_hit_rate < self.target_hit_rate - 0.1:
                # 命中率太低，降低阈值（更容易缓存）
                self.current_threshold *= (1 - self.adaptation_rate)
            elif current_hit_rate > self.target_hit_rate + 0.1:
                # 命中率太高，提高阈值（更严格）
                self.current_threshold *= (1 + self.adaptation_rate)
        ...
        if len(self.prediction_error_history) >= 5:
            ...
            if avg_error > 0.05:  # 预测误差较大
                self.current_threshold *= 1.05  # 稍微提高阈值，更保守
        
        # 保持合理范围
        self.current_threshold = np.clip(self.current_threshold, 0.001, 0.1)
    
    def get_threshold(self, timestep=None, block_importance=1.0):
        """获取当前阈值"""
        ...
        if timestep is not None:
            ...
            time_weight = 1.0 + 0.3 * (2 * abs(normalized_timestep - 0.5))
            threshold /= time_weight
        
        # 块重要性权重
        threshold /= block_importance
        
        return threshold

class SmartBlockCacheWithPrediction:
    """基于输入-输出预测的智能块缓存"""
    ...
    def __init__(self, block_index):
        ...
        # 核心组件
        self.predictor = InputOutputPredictor()
    ...
    def should_compute(self, modulated_input, force_compute=False, timestep=None):
        ...
        if should_compute:
            self.accumulated_distance = 0
            self.compute_count += 1
            reason = f"predicted_change_{predicted_output_change:.4f}_exceeds_threshold_{current_threshold:.4f}"
        else:
            self.cache_count += 1
            reason = f"predicted_change_{predicted_output_change:.4f}_below_threshold_{current_threshold:.4f}"
        
        # 更新previous input（无论是否计算都要更新）
        self.previous_modulated_input = modulated_input.clone()
    ...
    def update_with_actual_output(self, actual_output, actual_encoder_output=None):
        ...
        if self.previous_output is not None:
            ...
            if self.previous_modulated_input is not None:
                input_change = self._calculate_input_change(self.previous_modulated_input)
                ...
                # 更新阈值管理器
                was_cached = False  # 这里实际计算了，所以不是缓存
                self.threshold_manager.update(was_cached, prediction_error)
        ...
        self.cached_encoder_output = actual_encoder_output.clone() if actual_encoder_output is not None else None
...
def TeaCacheIntelligentForward(
        self,
    ...
    # 初始化智能缓存管理器
    if not hasattr(self, 'intelligent_caches'):
        self.intelligent_caches = {}
        self.intelligent_single_caches = {}
    ...
    for index_block, block in enumerate(self.transformer_blocks):
        if index_block not in self.intelligent_caches:
            self.intelligent_caches[index_block] = SmartBlockCacheWithPrediction(index_block)
        
        cache_manager = self.intelligent_caches[index_block]
    ...
    # 处理single transformer blocks（类似逻辑）
    for index_block, block in enumerate(self.single_transformer_blocks):
        if index_block not in self.intelligent_single_caches:
            self.intelligent_single_caches[index_block] = SmartBlockCacheWithPrediction(index_block)
        
        cache_manager = self.intelligent_single_caches[index_block]
        should_compute_block = force_compute
...
def get_intelligent_cache_summary(model):
    """获取智能缓存的性能摘要"""
    summary = {
        'transformer_blocks': {},
    ...
    if hasattr(model, 'intelligent_caches'):
        for block_idx, cache_manager in model.intelligent_caches.items():
    ...
    if hasattr(model, 'intelligent_single_caches'):
        for block_idx, cache_manager in model.intelligent_single_caches.items():
            stats = cache_manager.get_cache_statistics()
